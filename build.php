<?php
    ...
    */
    if (sizeof($argv) == 2 && ($argv[1] == "-h" || $argv[1] == "--help"))   {
        ...
        echo "\n**        -s input-schema [--sq optional-sequece-number] [--nosql any-value]  **";
        echo "\n**                                                                            **";
        ...
        echo "\n**nosql: Do not generate sql table [ only codes ]                             **";
        echo "\n**version: use --version or -v                                                **";
    ...
    $staticFile = $staticFolder . "__default_static_files__.php";
    ...
    for ($i=1; $i< sizeof($argv)-1; $i=$i+2)    {
        ...
        } else if (! $nosql && in_array($control, array("--nosql")))    {
            $nosql = true;
        } else if (($data != $version) && in_array($control, ["-v", "--version"]))    {
            $version = $data;
        }
    }
    ...
    try {
        ...
        SYSBuild::build($initFolder, $targetFolder, $inputSchema, $staticFolder, $staticFile,  $classesFolder, $jsonContent, $defaultIndexDocumentationFile, $accessForbidenFolder, $seqNumber, $nosql, $version);
    } catch (Exception $e)  {
        die($e->getMessage());
    }
?>