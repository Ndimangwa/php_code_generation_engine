...
<?php
    ...
    try {
        ...
        $doctor1 = new MedicalDoctor("Default", $_REQUEST['id'], $conn);
        if (isset($_POST['submit']))   {
            if ($_POST['efilter'] != $doctor1->getExtraFilter())  throw new Exception("System Detected a Browser Replay");
            $doctor1->setExtraFilter(__object__::getCodeString(32))->setSpecialist(isset($_POST['specialist']) ? 1 : 0)->update();
            $login1 = $doctor1->getLogin();
            $login1->updateList($_POST);
            $login1->update();
?>
        <div class="document-creator m-2">
            <?= __data__::showPrimaryAlert("You have successful updated the Medical Doctor") ?>
        </div>
<?php
        } else {
            $formToDisplay = __data__::createDataCaptureForm($nextPage, "MedicalDoctor", array(
                ...
                "page" => $page,
                "id" => $_GET['id'],
                "efilter" => $doctor1->getExtraFilter(),
                "submit" => true
...
?>
                ...
                <div class="card-footer">
                    <div class="text-center">
                        ...
                        <span class="text-muted"><i>Rule: medicaldoctor_update</i></span>
                    </div>
                </div>
            </div>
        </div>
